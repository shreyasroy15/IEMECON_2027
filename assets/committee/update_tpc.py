...
user_input = """Aaditeshwar Seth	IIT Delhi, India
...
for line in user_input.split('\n'):
    ...
    # Some lines might have a number at the beginning (e.g. "3\tAbdul Gafur Shaik\tIIT Jodhpur, India")
    # Let's split by tab.
    parts = line.split('\t')
    if len(parts) == 3:
        # Number, name, affiliation
        _, name, affiliation = parts
    elif len(parts) == 2:
        # Name, affiliation (like the first one)
        # Check if first part contains a number followed by space
