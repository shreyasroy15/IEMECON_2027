...
for line in raw_list.split('\n'):
    ...
    parts = line.split('\t')
    if len(parts) == 3:
        # e.g. "3\tAbdul Gafur Shaik\tIIT Jodhpur, India"
        names_to_remove.append(parts[1].strip())
    elif len(parts) == 2:
        # e.g. "Aaditeshwar Seth\tIIT Delhi, India"
        names_to_remove.append(parts[0].strip())
    else:
        # Just in case there are no tabs
        names_to_remove.append(line.strip())
        ...
        if '<td class="s-no">' in line and '<td class="member-name">' in line:
            ...
            if match:
                ...
                for n in names_to_remove:
                    if n == member_name or n in member_name or member_name in n:
                        should_remove = True
                        break
                if should_remove:
                    removed += 1
